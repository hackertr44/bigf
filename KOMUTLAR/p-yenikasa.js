...
exports.run = async (client, message,args) => {
  ...
  if (db.fetch(`goldpuan_${message.author.id}`) >= 75000) {
let kod31 = "";
  let kod = "1"
  for(var k = 0; k < 1; k++) {
    kod31 = (kod31 + kod.charAt(Math.floor(Math.random() * kod.length)));
  }
 const kod32 = require("useful-tools").hesapla(`${db.fetch(`goldpuan_${message.author.id}`)}-75000`)
 ...
 db.set(`goldpuan_${message.author.id}`, kod32)
  ...
  Jimp.read(`https://cdn.discordapp.com/attachments/649581228835602432/650052923728068628/unknown.png`, async(err, image) => {
    await image.resize(794, 598)
        ...
        await image.print(font, 220, 309, kod31 + " SINIRSIZ");
    Jimp.read(user.avatarURL, async(err, avatar) => {
      ...
      await image.composite(avatar, 269, 39).write(`./resimler/gold/gold-${message.author.id}.png`);
        setTimeout(function() {
            message.reply(`Bravo! **sınırsız Kasadan! (${kod31})** Vip Kazandın Azim İle Yapamıyacağın Pek Birşey Yok Sanırım Vip Verdim Artık Kullanabilirsin Hayırlı Olsun.`)
