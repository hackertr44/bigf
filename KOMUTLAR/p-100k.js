...
exports.run = async (client, message,args) => {
  ...
  if (db.fetch(`goldpuan_${message.author.id}`) >= 100000) {
let kod31 = "";
  let kod = "12345678910"
  for(var k = 0; k < 3; k++) {
  ...
  Jimp.read(`https://cdn.discordapp.com/attachments/649581228835602432/650052923728068628/unknown.png`, async(err, image) => {
    ...
    Jimp.read(user.avatarURL, async(err, avatar) => {
      ...
      await image.composite(avatar, 269, 39).write(`./resimler/gold/gold-${message.author.id}.png`);
        setTimeout(function() {
            message.reply(`Bravo! **Topaz Kasadan! (${kod31})** Gün Gold Üye Kazandın Azim İle {LEGEND} Yapamıyacağın Pek Birşey Yok Sanırım Gold Üyeni Verdim Artık Kullanabilirsin Hayırlı Olsun.`)
